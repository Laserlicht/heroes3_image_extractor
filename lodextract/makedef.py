...
def makedef(infile, outdir):
    ...
    fw,fh,pal = cursig
    numframes = sum(len(l) for l in list(infiles.values()))
    ...
    if not pal:
        ...
        for bid,l in list(infiles.items()):
            newl = []
            for lm,tm,im in l:
                w,h = im.size
                if w == 0 or h == 0:
    ...
    bidtablesize = 16*len(infiles)+sum(len(l)*(13+4) for l in list(infiles.values()))
    ...
    for bid,l in list(infiles.items()):
        ...
        for i,_ in enumerate(l):
            fn = b"%02d_%03d.pcx"%(bid,i)
            outf.write(struct.pack("13s", fn))
        # write data offsets
        for w,h,_,_,data,size in l:
            outf.write(struct.pack("<I",int(curoffset)))
    ...
    for bid,l in list(infiles.items()):
        for w,h,lm,tm,data,size in l:
            ...
            if fmt == 0:
                ...
                outf.write(struct.pack("<IIIIIIii",int(size),fmt,fw,fh,w,h,int(lm),tm))
                ...
                for d in data:
                    for e in d:
                        ...
                        if offs > ushrtmax:
                            print("exceeding max ushort value: %d"%offs)
                            return False
                        lineoffs.append(int(offs))
                        acc += len(e)
                outf.write(struct.pack("<"+"H"*int(w/32)*h, *lineoffs))
                for d in data: # line
